...
| `/tdd` | 必须先执行 `/on`，Issue 必须存在 | 可选：需要先写测试时使用 |
| `/fix` | 必须先执行 `/on`，Issue 必须存在 | 必选：实现逻辑，可直接使用或在 `/tdd` 之后使用 |
| `/review` | 必须先执行 `/on` |
| `/revise` | 必须先执行 `/on` |
...
| 模式 | 说明 |
|------|------|
| 直接 Fix | 没有测试用例的场景，直接实现逻辑 |
| Fix after TDD | `/tdd` 之后继续同一个 PR，实现逻辑让测试通过 |
...
- `/fix` 可以单独使用，也可以跟在 `/tdd` 之后
- 如果在 `/tdd` 之后，在同一个 PR 上继续工作
- 直到测试全部通过才进入 review
- 可以多次 `/fix`
